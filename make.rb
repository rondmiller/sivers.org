...
def parent_for(uri)
	...
	when AYW.include?(uri)
		'from the book “<a href="/a" rel="tag">Anything You Want</a>”'
	when YourMusic.include?(uri)
		'from the book “<a href="/m" rel="tag">Your Music and People</a>”'
	when HellYeah.include?(uri)
		'from the book “<a href="/n" rel="tag">Hell Yeah or No</a>”'
	else
...
@pagedescription = 'notes from over 250 books'
@bodyid = 'booklist'
@url = 'book'
html = template('header')
...
Dir['content/tweets/20*'].sort.each do |infile|
	...
	date = pf[:date]
	d = Date.parse(date)
	tweet = autolink(ERB::Util.html_escape(File.read(infile).strip))
	# save to array for later use in index and home page
...
<url><loc>https://sivers.org/now</loc><lastmod>#{today}</lastmod><changefreq>daily</changefreq><priority>0.9</priority></url>
<url><loc>https://sivers.org/about</loc><lastmod>#{today}</lastmod><changefreq>daily</changefreq><priority>0.9</priority></url>
...
<url><loc>https://sivers.org/i</loc><lastmod>#{today}</lastmod><changefreq>monthly</changefreq><priority>0.6</priority></url>
<url><loc>https://sivers.org/book</loc><lastmod>#{today}</lastmod><changefreq>monthly</changefreq><priority>0.7</priority></url>
<url><loc>https://sivers.org/projects</loc><lastmod>#{today}</lastmod><changefreq>monthly</changefreq><priority>0.3</priority></url>
<url><loc>https://sivers.org/presentations</loc><lastmod>#{today}</lastmod><changefreq>yearly</changefreq><priority>0.3</priority></url>
XML
@urls.sort.each do |u|
	xml << "<url><loc>https://sivers.org/#{u}</loc></url>\n"
end
xml << '</urlset>'
write_if_changed('site/sitemap.xml', xml)
